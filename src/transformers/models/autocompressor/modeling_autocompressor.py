...
class SummaryConfig:
    ...
    soft_prompt_length: int = 0
    past_key_values_soft_prompt_length: int = 0
    summary_length: int = 0

    def reset(self):
        self.soft_prompt_length = 0
        self.past_key_values_soft_prompt_length = 0
...
class AutoCompressorMixin:
    ...
    def forward_segment(
        ...
        past_key_values_soft_prompt_length: int
    ):

        bsz = segment_embeds.size(0)
        summary_length = summary_token_embeds.size(1)
        if past_key_values_soft_prompt_length > 0: # soft_prompt should already be in past_key_values
            ...
            segment_attention_mask = torch.cat([
                torch.ones(bsz, past_key_values_soft_prompt_length, device=device, dtype=attn_dtype),
            ...
            device, attn_dtype = segment_embeds.device, segment_attention_mask.dtype
            segment_attention_mask = torch.cat([
                torch.ones(bsz, soft_prompt_length, device=device, dtype=attn_dtype),
                segment_attention_mask,
                torch.ones(bsz, summary_length, device=device, dtype=attn_dtype)
            ], dim=1)
        ...
        def decoder(segment_embeds,
                    ...
                    soft_prompt_length,
                    past_key_values_soft_prompt_length,
                    summary_length):
            self.summary_config.soft_prompt_length = soft_prompt_length
            self.summary_config.past_key_values_soft_prompt_length = past_key_values_soft_prompt_length
        ...
        if segment_gradient_checkpointing:
            outputs = torch.utils.checkpoint.checkpoint(
                ...
                soft_prompt_length, past_key_values_soft_prompt_length, summary_length,
                use_reentrant=False)
        else:
            outputs = decoder(
        ...
        segment_last_hiddens = (
            outputs.last_hidden_state[:, soft_prompt_length:total_length - summary_length]
        )
        new_soft_prompt = outputs.last_hidden_state[:, total_length - summary_length:]

        return outputs, segment_last_hiddens, new_soft_prompt
    ...
    def forward(
        ...
        # We formulate the past_key_values as a tuple where the second entry is the soft_prompt already in the past key values
        if past_key_values is not None and isinstance(past_key_values, dict):
            # Replace soft_prompt in direct argument with the soft_prompt in past_key_values
            past_key_values, soft_prompt = past_key_values["past_key_values"], past_key_values["soft_prompt"]
            past_key_values_soft_prompt_length = soft_prompt.size(1)
        else:
            past_key_values_soft_prompt_length = 0

        past_key_values_length = self.get_past_key_values_len(past_key_values) - past_key_values_soft_prompt_length
        ...
        # With past_key_values we will process the input in a single pass (for generation), except when generting summary vectors
            ...
            if use_cache and past_key_values_length + inputs_embeds.size(1) == segment_lengths:
                ...
                # If we use cache and output soft_prompt, we need to add a dummy segment to the end to get the past key values of the soft_prompt
                ...
                summary_token_embeds_list = (summary_token_embeds if output_soft_prompt else summary_token_embeds[:,:0,:],)
        ...
        for step, summary_token_embeds in enumerate(summary_token_embeds_list):
            ...
            outputs, segment_hidden_states, new_soft_prompt = self.forward_segment(
                soft_prompt.to(inputs_embeds.dtype), inputs_embeds_list[step], summary_token_embeds, attention_mask_list[step],
                past_key_values, output_hidden_states, use_cache, output_attentions,
                segment_gradient_checkpointing, past_key_values_soft_prompt_length)
        ...
        output = CausalACOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values={"past_key_values": past_key_values, "soft_prompt": soft_prompt},
...
class AutoCompressorForCausalLM(AutoCompressorMixin, LlamaForCausalLM):
